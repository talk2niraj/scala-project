 ...
 */
 class ListsSuite {
   ...
   * when writing tests.
   */
  @Test def `details why one plus one plus one is three (0pts)`: Unit = {
  ...
  @Test def `sum of a few numbers [above test written with Assert.assertEquals] (10pts)`: Unit = {
    Assert.assertEquals(sum(List(1,2,0)), 3)
  }

  @Test def `sum of elements in the empty list is 0 (10pts)`: Unit = {
    Assert.assertEquals(sum(List()), 0)
  }

  @Test def `sum 1, 2, and 3 is not 3 (10pts)`: Unit = {
    assert(sum(List(1,2,3)) != 3)
  }

  @Test def `sum of a few negative numbers (10pts)`: Unit = {
    assert(sum(List(-4, -3, -7)) == -14)
  }
  ...
  @Test def `max of few numbers in the list (10pts)`: Unit = {
    Assert.assertEquals(max(List(3, 7, 2)), 7)
  }

  @Test def `max is same element if it is the only one element in the list (10pts)`: Unit = {
    Assert.assertEquals(max(List(10)), 10)
  }

  @Test def `max of few negative and positive numbers in the list (10pts)`: Unit = {
    Assert.assertEquals(max(List(3, -7, -2, 1, 4)), 4)
  }

  @Test def `max throws an exception if list is empty`: Unit = {
    try {
      Lists.max(List())
      Assert.fail("No exception has been thrown")
    } catch {
